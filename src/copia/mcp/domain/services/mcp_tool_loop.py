    ...
    def complete(self, messages: list[ChatMessage], config: LLMConfig) -> LLMResponse:
        ...
        while True:
            ...
            if not response.tool_calls:
                ...
                if correction:
                    if final_retry_count >= 1:
                        raise McpToolLoopError("MCP final response failed validation")
                    conversation.append(ToolLoopMessage(role="assistant", content=response.content))
                    conversation.append(ChatMessage(role="user", content=correction))
                    definitions = []
                    final_retry_count += 1
                    continue
                return response
